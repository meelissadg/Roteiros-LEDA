...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	private void grauDois(BSTNode<T> node){
		BSTNode<T> sucessorAux = sucessor(node.getData());
		...
		node.setData(sucessorAux.getData());
		
		if (retornaQuantFilhos(sucessorAux) == 0) {
			grauZero(sucessorAux);
		}
		else if (retornaQuantFilhos(sucessorAux) == 1) {
			grauUm(sucessorAux);
		}else{
			grauDois(sucessorAux);
		}
	...
	public void remove(T element) {
		...
		quantFilhos = retornaQuantFilhos(node);
