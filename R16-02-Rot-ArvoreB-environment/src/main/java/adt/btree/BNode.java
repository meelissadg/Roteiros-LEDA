...
public class BNode<T extends Comparable<T>> {
	...
	protected void split(){
		...
		int posicaoIrmao = this.getParent().indexOfChild(this) + 1;

		T elementoMeio = this.getElementAt(medio);
		BNode<T> meuIrmao = new BNode(this.getOrder());
		
		
		if(this.getParent() == null) {
			parent = new BNode<T>(this.order);
			parent.addChild(0, this);
		}
		
		this.getParent().addChild(posicaoIrmao, meuIrmao);

		for (int i = medio; i < this.size(); i++) {
			meuIrmao.addElement(this.getElementAt(i));
		}
		
		if(meuIrmao.size() != 0) {
			meuIrmao.removeElement(elementoMeio);
		}
	
		getParent().removeChild(this);
		promote(elementoMeio);
	}
	protected void promote(T element){
